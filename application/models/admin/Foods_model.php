...
class Foods_model extends CI_Model
{
	...
	/**
	 * caching of users and their foods
	 *
	 * @var array
	 */
	public $_cache_user_in_food = array();
	 ...
	 * @author    Mathew
	 */
	public function register($identity, $password, $email, $additional_data = array(), $foods = array())
	{
		...
		else if (!$this->config->item('default_food', 'ion_auth') && empty($foods))
		{
			$this->set_error('account_creation_missing_default_food');
			return FALSE;
		}

		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['foods'], array('name' => $this->config->item('default_food', 'ion_auth')), 1)->row();
		if (!isset($query->id) && empty($foods))
		{
			$this->set_error('account_creation_invalid_default_food');
			return FALSE;
		}

		// capture default food details
		$default_food = $query;
		...
		// add in foods array if it doesn't exists and stop adding into default food if default food ids are set
		if (isset($default_food->id) && empty($foods))
		{
			$foods[] = $default_food->id;
		}
	 ...
	 * @author Ben Edmunds
	 */
	public function users($foods = NULL)
	{
		...
		// filter by food id(s) if passed
		if (isset($foods))
		{
			// build an array if only one food was passed
			if (!is_array($foods))
			{
				$foods = Array($foods);
			}

			// join and then run a where_in against the food ids
			if (isset($foods) && !empty($foods))
			{
				$this->db->distinct();
				$this->db->join(
				    $this->tables['users_foods'],
				    $this->tables['users_foods'].'.'.$this->join['users'].'='.$this->tables['users'].'.id',
				    'inner'
				);
			}

			// verify if food name or food id was used and create and put elements in different arrays
			$food_ids = array();
			$sTenThucAns = array();
			foreach($foods as $food)
			{
				if(is_numeric($food)) $food_ids[] = $food;
				else $sTenThucAns[] = $food;
			}
			$or_where_in = (!empty($food_ids) && !empty($sTenThucAns)) ? 'or_where_in' : 'where_in';
			// if food name was used we do one more join with foods
			if(!empty($sTenThucAns))
			{
				$this->db->join($this->tables['foods'], $this->tables['users_foods'] . '.' . $this->join['foods'] . ' = ' . $this->tables['foods'] . '.id', 'inner');
				$this->db->where_in($this->tables['foods'] . '.name', $sTenThucAns);
			}
			if(!empty($food_ids))
			{
				$this->db->{$or_where_in}($this->tables['users_foods'].'.'.$this->join['foods'], $food_ids);
			}
		}
	 ...
	 * @author Ben Edmunds
	 */
	public function get_users_foods($id = FALSE)
	{
		$this->trigger_events('get_users_food');

		// if no id was passed use the current users id
		$id || $id = $this->session->userdata('user_id');

		return $this->db->select($this->tables['users_foods'].'.'.$this->join['foods'].' as id, '.$this->tables['foods'].'.name, '.$this->tables['foods'].'.description')
		                ->where($this->tables['users_foods'].'.'.$this->join['users'], $id)
		                ->join($this->tables['foods'], $this->tables['users_foods'].'.'.$this->join['foods'].'='.$this->tables['foods'].'.id')
		                ->get($this->tables['users_foods']);
	}

	/**
	 * add_to_food
	 *
	 * @param array|int|float|string $food_ids
	 * @param bool|int|float|string  $user_id
	 *
	 * @return int
	 * @author Ben Edmunds
	 */
	public function add_to_food($food_ids, $user_id = FALSE)
	{
		$this->trigger_events('add_to_food');
		...
		foreach ($food_ids as $food_id)
		{
			...
			if ($this->db->insert(
								  $this->tables['users_foods'],
								  array(
								  	$this->join['foods'] => (float)$food_id,
									$this->join['users']  => (float)$user_id
								  )
								)
			)
			{
				if (isset($this->_cache_foods[$food_id]))
				{
					$sTenThucAn = $this->_cache_foods[$food_id];
				}
				else
				{
					$food = $this->food($food_id)->result();
					$sTenThucAn = $food[0]->name;
					$this->_cache_foods[$food_id] = $sTenThucAn;
				}
				$this->_cache_user_in_food[$user_id][$food_id] = $sTenThucAn;

				// Return the number of foods added
				$return++;
			}
		}
	 ...
	 * remove_from_food
	 *
	 * @param array|int|float|string|bool $food_ids
	 * @param int|float|string|bool $user_id
	 *
	 * @return bool
	 * @author Ben Edmunds
	 */
	public function remove_from_food($food_ids = FALSE, $user_id = FALSE)
	{
		$this->trigger_events('remove_from_food');
		...
		// if food id(s) are passed remove user from the food(s)
		if (!empty($food_ids))
		{
			if (!is_array($food_ids))
			{
			...
			foreach ($food_ids as $food_id)
			{
				...
				$this->db->delete(
					$this->tables['users_foods'],
					array($this->join['foods'] => (float)$food_id, $this->join['users'] => (float)$user_id)
				);
				if (isset($this->_cache_user_in_food[$user_id]) && isset($this->_cache_user_in_food[$user_id][$food_id]))
				{
					unset($this->_cache_user_in_food[$user_id][$food_id]);
				}
			}

			$return = TRUE;
		}
		// otherwise remove user from all foods
		else
		{
			// Cast to float to support bigint data type
			if ($return = $this->db->delete($this->tables['users_foods'], array($this->join['users'] => (float)$user_id)))
			{
				$this->_cache_user_in_food[$user_id] = array();
			}
		}
		return $return;
	}
	...
	public function foods()
	{
		...
		$this->response = $this->db->get($this->tables['cal_thucan']);
	...
	{
		$this->trigger_events('food');

		if (isset($id))
		{
			$this->where($this->tables['cal_thucan'].'.iIDThucAn', $id);
		}

		$this->limit(1);
		$this->order_by('iIDThucAn', 'desc');
	 ...
	 * @author Ben Edmunds
	 */
	public function search_foods($keyword = NULL)
	{
		$this->db->like('sTenThucAn',$keyword);
		$query  =   $this->db->get('cal_thucan');
	...
	public function delete_user($id)
	{
		...
		// remove user from foods
		$this->remove_from_food(NULL, $id);

		// delete user from users table should be placed after remove from food
	 ...
	 * @param array       $additional_data
	 *
	 * @return int|bool The ID of the inserted food, or FALSE on failure
	 * @author aditya menon
	 */
	public function create_food($sTenThucAn = FALSE, 
								$sSLplusDVT = '', 
								$fCalori = 0, 
								$fDam = 0, 
								$fBeo = 0, 
	...
	{
		// bail if the food name was not passed
		if(!$sTenThucAn)
		{
			$this->set_error('sTenThucAn_required');
			return FALSE;
		}
		

		// bail if the food name already exists
		$existing_food = $this->db->get_where($this->tables['cal_thucan'], array('sTenThucAn' => $sTenThucAn))->num_rows();
		if($existing_food !== 0)
		{
			$this->set_error('food_already_exists');
			return FALSE;
		}
		...
		// filter out any data passed that doesnt have a matching column in the foods table
		// and merge the set food data and the additional data
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['cal_thucan'], $additional_data), $data);

		$this->trigger_events('extra_food_set');

		// insert the new food
		$this->db->insert($this->tables['cal_thucan'], $data);
		$iIDThucAn = $this->db->insert_id($this->tables['cal_thucan'] . '_id_seq');

		// report success
		$this->set_message('food_creation_successful');
		// return the brand new food id
		return $iIDThucAn;
	}
	 ...
	 * @author aditya menon
	 */
	public function update_food($iIDThucAn = FALSE, 
								$sTenThucAn = FALSE, 
								$sSLplusDVT = '', 
								$fCalori = 0, 
								$fDam = 0, 
								...
								$additional_data = array())
	{
		...
		if (!empty($sTenThucAn))
		{
			...
			// bail if the food name already exists
			$existing_food = $this->db->get_where($this->tables['cal_thucan'], array('sTenThucAn' => $sTenThucAn))->row();
			if (isset($existing_food->iIDThucAn) && $existing_food->iIDThucAn != $iIDThucAn)
			{
				$this->set_error('food_already_exists');
				return FALSE;
			}
		...
		// // restrict change of name of the admin food
		// $food = $this->db->get_where($this->tables['cal_thucan'], array('iIDThucAn' => $iIDThucAn))->row();
		// if ($this->config->item('admin_food', 'ion_auth') === $food->name && $sTenThucAn !== $food->sTenThucAn)
		// {
		...
		$this->db->update($this->tables['cal_thucan'], $data, array('iIDThucAn' => $iIDThucAn));

		$this->set_message('food_update_successful');
	 ...
	 * delete_food
	 *
	 * @param int|string|bool $food_id
	 *
	 * @return bool
	 * @author aditya menon
	 */
	public function delete_food($iIDThucAn = FALSE)
	{
		// bail if mandatory param not set
		if(!$iIDThucAn || empty($iIDThucAn))
		{
			return FALSE;
		}
		$this->db->where('iIDThucAn', $iIDThucAn);
		$this->db->delete('cal_thucan');
		return TRUE;
		
		// $food = $this->food($food_id)->row();
		// if($food->name == $this->config->item('admin_food', 'ion_auth'))
		// {
		// 	$this->trigger_events(array('post_delete_food', 'post_delete_food_notallowed'));
		// 	$this->set_error('food_delete_notallowed');
		// 	return FALSE;
		// }

		// $this->trigger_events('pre_delete_food');

		// $this->db->trans_begin();

		// // remove all users from this food
		// $this->db->delete($this->tables['users_foods'], array($this->join['foods'] => $food_id));
		// // remove the food itself
		// $this->db->delete($this->tables['foods'], array('id' => $food_id));

		// if ($this->db->trans_status() === FALSE)
		// {
		// 	$this->db->trans_rollback();
		// 	$this->trigger_events(array('post_delete_food', 'post_delete_food_unsuccessful'));
		// 	$this->set_error('food_delete_unsuccessful');
		// 	return FALSE;
		// }

		// $this->db->trans_commit();

		// $this->trigger_events(array('post_delete_food', 'post_delete_food_successful'));
		// $this->set_message('food_delete_successful');
		// return TRUE;
	}
